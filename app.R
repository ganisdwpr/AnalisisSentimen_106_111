...
ui <- dashboardPage(
  skin="purple",
  dashboardHeader(title = "Review Restoran Yogyakarta"),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Dashboard", tabName = "dashboard", icon = icon("home")),
      menuItem("Perbandingan Restoran", tabName = "da", icon = icon("poll"))
    ), 
    fluidPage(
      hr(),
      helpText(
        "Data review restoran hasil dari web-scarpping ",
  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(
        ...
        fluidRow(
          box(
            title = "Restoran Review dan Klasifikasi Sentimen",
            solidHeader = T,
            width = 12,
            collapsible = T,
...
server <- function(input, output) {
  ...
  #memanggil restoran yang dipilih  
  data <- reactive({
